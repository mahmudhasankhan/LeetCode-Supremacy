...
SELECT results 
FROM (SELECT TOP 1 name AS results
      FROM Users
      WHERE user_id in 
	    (SELECT temp.user_id FROM (
			SELECT user_id, count(movie_id) as movie_count,
			DENSE_RANK() OVER(ORDER BY count(movie_id) DESC) AS rownum
			FROM MovieRating
...
FROM (SELECT TOP 1 title AS results
      FROM Movies
      WHERE movie_id in 
	    (SELECT temp2.movie_id
		FROM (SELECT movie_id, 
		      AVG(CAST(rating AS DECIMAL)) AS avg_rating,
		      DENSE_RANK() OVER(ORDER BY AVG(CAST(rating AS DECIMAL)) DESC) AS rownum
		      FROM MovieRating
		      WHERE MONTH(created_at) = 2
                      AND YEAR(created_at) = 2020
		      GROUP BY movie_id) temp2
	    WHERE temp2.rownum = 1)
      ORDER BY title) X

-- Not a hard problem at all but annoying af, especially the edge cases
