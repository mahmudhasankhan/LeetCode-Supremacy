...
SELECT 
ROUND(
  CAST(SUM(CASE WHEN fo.min_date = d.customer_pref_delivery_date THEN 1 ELSE 0 END) AS DECIMAL)
  /
  CAST(COUNT(DISTINCT fo.customer_id) AS DECIMAL) * 100, 2) AS immediate_percentage
FROM first_orders AS fo 
JOIN Delivery as d
ON fo.customer_id = d.customer_id
AND fo.min_date = d.order_date

--Runtime: 2543ms Beats 15.22% of users with MS SQL Server
